...
def main(start, stop, incr, desc):
    ...
    instrs = {stage_cam.serial: stage_cam}
    for i in range(N):
        ...
        adjust_shutter(DAQ, pos, stage_cam)
...
def adjust_shutter(DAQ, pos, cam):
    z = [0, 2200]
    shutter = [2.1, 10]
    shutter = interp1d(z, shutter)
    new_shut = shutter(pos)
    DAQ.send_command(cam, 'set_shutter', float(new_shut))
    pass
