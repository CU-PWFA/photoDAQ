...
qtCreatorFile = "windows/jitter.ui"
...
class UI(QtBaseClass, Ui_MainWindow):
    ...
    def __init__(self, DAQ):
        ...
        self.cameraSet.addItems(['', 'DM Near Far', 'Probe Near Far'])
        self.cameraSet.currentTextChanged.connect(self.connectCalledCameras)
        
        
    def connectCalledCameras(self, desc):
        ...
        if desc== 'DM Near Far':
            ...
            if '18085415' in cameras:
                Near = cameras['18085415']
                print('DMNear detected')
            else:
                print('DMNear not detected')
            if '18085362' in cameras:
                Far = cameras['18085362']
                print('DMFar detected')
            else:
                print('DMFar not detected')

            print('Connecting cameras...')
            DAQ.connect_instr(Near)
            DAQ.connect_instr(Far)

        elif desc== 'Probe Near Far':
            print('Detecting cameras...')
            cameras = detect.camera()
            if '17583372' in cameras:
                Near = cameras['17583372']
                print('ProbeNear detected')
            else:
                print('ProbeNear not detected')
            if '24070909' in cameras:
                Far = cameras['24070909']
                print('ProbeFar detected')
            else:
                print('ProbeFar not detected')
    ...
    def connect_instruments(DAQ):
        ...
        cameras = detect.camera()
        if '18085415' in cameras:
            DM_NF = cameras['18085415']
        else:
            print('DM NF camera (serial 18085415) not detected')
            return
        if '18085362' in cameras:
            FF = cameras['18085362']
        else:
